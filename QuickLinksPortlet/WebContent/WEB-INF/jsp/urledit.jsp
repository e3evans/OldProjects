...
<form:form  commandName="urlFormBean" method="post" name="editUrlForm" id="editUrlForm" action="${formAction}">
   ...
   <c:forEach items="${urlFormBean.listUrlBean}" var="urlLst" varStatus="row">
      <tr>
      	...
      	<td><form:checkbox  path="listUrlBean[${row.index}].checked" id="listUrlBean[${row.index}].checked" />
										</td>
										<td><form:hidden path="listUrlBean[${row.index}].appName" value="${urlLst.appName}" /><c:out value="${urlLst.appName}"/></td>
      	<td><form:hidden path="listUrlBean[${row.index}].appDesc" value="${urlLst.appDesc}" /><c:out value="${urlLst.appDesc}"/>
      	<%-- <form:hidden path="listUrlBean[${row.index}].userid" value="${urlLst.userid}" />
      	<c:out value="${urlLst.appURL}"/></td> --%>
