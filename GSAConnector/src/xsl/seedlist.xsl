  ...
  <xsl:template match="atom:entry">
	  <record action="add" mimetype="text/html">
		  <xsl:attribute name="displayurl">
			  <xsl:for-each select="atom:link">http://caregiverconnect-dev.aurora.org<xsl:value-of select="@href"/></xsl:for-each>
		  </xsl:attribute>
		  <xsl:attribute name="url">
			  <xsl:for-each select="atom:content">http://caregiverconnect-dev.aurora.org<xsl:value-of select="@src"/></xsl:for-each>
		  </xsl:attribute>
		  <metadata>
