...
import org.aurora.quicklinksservices.exceptions.PermissionsUnavailableException;
import org.aurora.quicklinksservices.utils.SSOUtil;
...
public class PortalUser implements Serializable {
	 ...
	 * 
	 * @param util
	 *            utility used when parsing SSO cookie
	 */
	public PortalUser(SSOUtil util) {
		if (util != null) {
			String portal = util.getPortal();
			String strPortalKey = SSOUtil.getElementValue(portal,
					SSOUtil.PORTAL_KEY);
	...
	public String getFullName() {
		return new StringBuffer().append(getFirstName()).append(" ")
				.append(getLastName()).toString();
	}

	/**
	 * Checks if a permission exists. Throws an exception if the permissions are
	 * unknown (which could happen if there was a portal-services problem that
	 * resulted in the PortalSSOFilter being unable to load the data, or if the
	 * filter was not configured with load-permissions=true). This method checks
	 * for the existence of the permission (aka: AppAuth) across all
	 * sub-applications. The sequence numbers are disregarded.
	 * 
	 * @param appAuthName
	 *            The AppAuth name.
	 * @return True if the permission exists.
	 * @throws PermissionsUnavailableException
	 *             Thrown if the permissions are not available.
	 */
	public boolean hasPermission(final String appAuthName)
			throws PermissionsUnavailableException {
		return hasPermission(appAuthName, null, null);
	}

	/**
	 * Checks if the permission exists. This method signature is provided
	 * because, in general, only the AppAuths (the permissions) of the current
	 * application exist in the PortalUser object because the appId is supplied
	 * as a configuration parameter to PortalSSOFilter. Hence, the appId
	 * parameter is not required.
	 * 
	 * @param appAuthName
	 *            The AppAuth name.
	 * @param seqNo
	 *            Optional parameter. If non-null, only AppAuths with a matching
	 *            Sequence Number are considered.
	 * @return True if the permission exists.
	 * @throws PermissionsUnavailableException
	 *             Thrown if the permissions are not available.
	 */
	public boolean hasPermission(final String appAuthName, final Integer seqNo)
			throws PermissionsUnavailableException {
	 ...
	 *             Thrown if the permissions are not available.
	 */
	public boolean hasPermission(final String appAuthName, final String appId,
			final Integer seqNo) throws PermissionsUnavailableException {
		if (this.permissions != null) {
			final Iterator iter = this.permissions.iterator();
			while (iter.hasNext()) {
				AppAuthInfo appAuth = (AppAuthInfo) iter.next();
				boolean hasPermission = appAuthName.equalsIgnoreCase(appAuth
						.getAppAuthName());
				hasPermission &= appId == null
						|| appId.equals(appAuth.getAppId());
				hasPermission &= seqNo == null
						|| seqNo.equals(appAuth.getSeqNo());
				if (hasPermission) {
					return true;
				}
			}
			...
			throw new PermissionsUnavailableException(
					"Permissions are not currently available. Please check that "
	...
	/**
	 * Returns true if PermissionsUnavailableException was thrown at least once
	 * since the last time {@link #setPermissions(Collection)} was called.
	 * 
	...
	/**
	 * Sets the permissions of this PortalUser. The corresponding getter method
	 * is intentionally missing because the {@link #hasPermission(String)}
	...
	/**
	 * Performs a shallow copy of the other PortalUser object's permissions.
	 * This was needed because the PortalSSOFilter creates a new PortalUser
	 * instance for each request, but we want to avoid loading the permissions
	 * from the {@link org.aurora.portal_services.services.UserService} each
	 * time.
	 * 
	 * @param anotherUser
	 *            The other PortalUser object.
	 */
