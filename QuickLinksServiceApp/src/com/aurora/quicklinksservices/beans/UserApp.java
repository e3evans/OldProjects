  ...
  implements Serializable
{
 ...
 /* public Set getUserAppParams() {
    return this.userAppParams;
  }

  public void setUserAppParams(Set userAppParams) {
    this.userAppParams = userAppParams;
  }

  public void addUserAppParam(UserAppParam userAppParam) {
    userAppParam.setUserApp(this);
    this.userAppParams.add(userAppParam);
  }
...
*/
  /*public UserAppParam getUserAppParam(String paramName) {
    UserAppParam userAppParam = null;
    for (Iterator i = this.userAppParams.iterator(); i.hasNext(); ) {
      UserAppParam tmpUserAppParam = (UserAppParam)i.next();
      if (tmpUserAppParam.getUserAppParamKey().getParamName().equals(paramName)) {
        userAppParam = tmpUserAppParam;
      }
    }
    return userAppParam;
  }*/
  ...
  @ManyToOne(fetch = FetchType.LAZY)
  @JoinColumns({
	  @JoinColumn(name="PT2J_USERID",updatable=false,insertable=false)
	   })
  public User getUser()
  {
    return this.user;
